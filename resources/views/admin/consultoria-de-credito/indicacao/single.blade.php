...
@stop
@section('js')
<script src="{{url('js/paginas/consultoria_indicacao.js')}}"></script>
...
<div class="row">
    <div class="col-md-12">
        <a href="#" class="btn btn-primary pull-right btn_novo_negocio" data-toggle="modal" data-target="#consultoriaModal">
        ...
        <div class="row">
            <div class="col-xs-12">
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th width="15%">Renda comprovada</th>
                            <th width="15%">Valor do imóvel</th>
                            <th width="15%">Valor do consultoria</th>
                            <th width="25%">Informações</th>
                            <th width="10%">Data</th>
                    ...
                    <tbody>
                        ...
                        <tr id="negocio-{{ $consultoria['id'] }}"
                        data-renda="{{ number_format($consultoria['renda_comprovada'], 2, ',', '.') }}"
                        data-bem="{{ number_format($consultoria['valor_bem'], 2, ',', '.') }}"
                        data-financiamento="{{ number_format($consultoria['valor_financiado'], 2, ',', '.') }}"
                        data-type="{{ $consultoria['type'] }}"
                        data-note="{{ $consultoria['note'] }}"
                        data-status="{{ $consultoria['status'] }}">
                            <td>R$ {{ number_format($consultoria['renda_comprovada'], 2, ',', '.') }}</td>
                            <td>R$ {{ number_format($consultoria['valor_bem'], 2, ',', '.') }}</td>
                            <td>R$ {{ number_format($consultoria['valor_financiado'], 2, ',', '.') }}</td>
                            <td>{{ (new \App\Helpers\Helper)->shortText($consultoria['note'], 30) }}</td>
                            <td>{{ $consultoria['date_formatada'] }}</td>
                            <td>{{ $consultoria['status_formatado'] }}</td>
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right" style="left: auto">
                                        <li>
                                        <a href="{{ $consultoria['id'] }}" class="btn_edita_negocio">
                                            <i class="fa fa-pencil"></i> Editar</a>
                                        </li>
                                        <li>
                                        <a href="{{ $consultoria['id'] }}" class="btn_ver_informacoes">
                                            <i class="fa fa-info"></i> Ver informações</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
...
<div id="consultoriaModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form action="{{ url('admin/consultoria-de-credito/indicacao/' . $array['type_slug'] . '/novo') }}" method="post" name="novo_consultoria">
              {!! csrf_field() !!}
            <input name="type" type="hidden" value="{{$array['type']}}">
            <input name="client_id" type="hidden" value="{{$cliente['id']}}">
            <input name="consultoria_id" type="hidden" value="">
                  ...
                  <div class="row">
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="text" name="valor_financiado" class="form-control valor" placeholder="Valor do financiamento">
                        </div>
                      </div>
                      <div class="col-md-12">
                            <div class="form-group">
